...
export interface ProjectRegisterState {
...
export function createProjectRegisterStore() {
  return createStore<ProjectRegisterState>((set) => ({
    projectId: null,
    gisuId: null,
    basicInfo: null,
...
export type ProjectRegisterStoreApi = ReturnType<
  typeof createProjectRegisterStore
>

const ProjectRegisterStoreContext =
  createContext<ProjectRegisterStoreApi | null>(null)

export function ProjectRegisterStoreProvider({
  children,
}: {
  children: ReactNode
}) {
  const storeRef = useRef<ProjectRegisterStoreApi | null>(null)
  if (storeRef.current === null) {
    storeRef.current = createProjectRegisterStore()
  }
  return (
    <ProjectRegisterStoreContext.Provider value={storeRef.current}>
      {children}
    </ProjectRegisterStoreContext.Provider>
  )
}

export function useProjectRegisterStoreApi(): ProjectRegisterStoreApi {
  const store = useContext(ProjectRegisterStoreContext)
  if (!store) {
    throw new Error(
      "useProjectRegisterStore는 ProjectRegisterStoreProvider 내부에서만 사용할 수 있습니다.",
    )
  }
  return store
}

export function useProjectRegisterStore<T>(
  selector: (state: ProjectRegisterState) => T,
): T {
  return useStore(useProjectRegisterStoreApi(), selector)
}
