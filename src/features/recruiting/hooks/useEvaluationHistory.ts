...
// 이력을 보여주게 된다.
//
// 필터·정렬·그룹핑은 화면이 클라이언트에서 처리하므로 전량을 받아 온다. 필터 바가
// 지부·학교를 이름으로 다루는데, 그 이름을 ID 로 옮기려면 이미 받아 둔 행에 실린 ID 가
// 있어야 한다(toDecisionHistoriesQuery 참고). 서버 필터로 옮기려면 이름 대신 ID 를
// 다루도록 필터 바부터 바꿔야 해서 별도 작업이다.
