...
describe("MemberCount", () => {
  it.each([
    ...
    ["lg", "text-subtitle-1-medium", "font-normal!"],
  ] as const)(
    "%s 크기에서 숫자와 슬래시의 피그마 텍스트 토큰을 적용한다",
    (size, numberClass, slashClass) => {
      render(<MemberCount current={3} total={10} size={size} />)

      expect(screen.getByText("3")).toHaveClass(numberClass)
      expect(screen.getByText("/")).toHaveClass(slashClass)
